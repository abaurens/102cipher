//
// Main.cpp for cipher_bonus in /home/bauren_a/Work/Tek1/Maths/102cipher/Bonus
// 
// Made by Arthur
// Login   <bauren_a@epitech.net>
// 
// Started on  Sat Nov 28 04:35:14 2015 Arthur
// Last update Sat Nov 28 16:19:36 2015 Arthur
//
...
int main(int ac, char **av)
{
  ...
  do
    {
      retry = false;
      key = new Matrix(keyStr);
      revKey = new Matrix(key->getWidth(), key->getHeight());
      canDecrypt = MatrixInversion(key->getTab(), key->getWidth(), revKey->getTab());
      if (!canDecrypt)
	{
	  char c;
	  cout << "[WARNING !] The key code \"" << keyStr << "\" can not be reversed. Do you want to change it ? (Y/N)" << endl;
	  cin >> c;
	  if (c == 'y' || c == 'Y')
	    {
	      cout << "Enter the new key code : ";
	      cin >> keyStr;
	      retry = true;
	      cout << "Retrying with key \"" << keyStr << "\"" << endl;
	    }
	}
    }
  while (retry);
